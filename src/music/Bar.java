...
public class Bar extends Mass { // 分隔符和结束符
    ...
    public Bar(Sys sys, int x) {
        ...
        this.sys = sys;
        this.x = x;
        addReaction(new Reaction("S-S") { // this is the cycle the bartype
            public int bid(Gesture gest) {
                int x = gest.vs.xM();
                if (Math.abs( x- Bar.this.x) > UC.barTooMarginSnap){return UC.noBid;}
    ...
    public void toggleLeft(){barType = barType^LEFT;} // exclusive or  ^ (switch 0 and 1)
    public void toggleRight(){barType = barType^RIGHT;}



    public void show(Graphics g){
        int sysTop = sys.yTop(), N = sys.fmt.size(), y1 = 0, y2 = 0; //y1, y2 is top and bottom of connect component
    ...
    public static void wings(Graphics g, int x, int y1, int y2, int dx, int dy){ //分隔符的歪出去
    ...
    public  void drawDots(Graphics g, int x, int top){
        ...
        if ((barType & RIGHT) != 0){
            g.fillOval(x + 2 * H, top + (11 * H) / 4, H / 2, H / 2);
            g.fillOval(x + 2 * H, top + (19 * H) / 4, H / 2, H / 2);
        }

...
public class Stem extends Duration implements Comparable<Stem>{
    ...
    private Stem(boolean up, Sys sys, Head.List heads){
        ...
        addReaction(new Reaction("E-E") { // increment flag on stem
            ...
            public int bid(Gesture gest) {
                ...
                return Math.abs(y- (y1 + y2) /2) + 51; // allow multi stem in sys to out bid this
            }
    ...
    public static Beam internalStem(Sys sys,int x, int y1, int y2) {
        for (Stem s : sys.stems) {
            if (s.beam != null && s.x() < x && s.yLo() < y2 && s.yHi() > y1) {
                ...
                if (Beam.verticalLineCrossLineSegment(x, y1, y2, bX, bY, eX, eY)) {
                    return s.beam;
                }
            }
        }
    ...
    public void setWrongSides(){
        ...
        int i, last, inc;
        if (isUp) {
            i = heads.size() - 1; last = 0; inc = - 1;
        }
        else{
            i = 0;last = heads.size()-1; inc = 1;
        }
        Head ph = heads.get(i);
        ph.wrongSide = false;
        while (i != last){
    ...
    public static class List extends ArrayList<Stem> {
        ...
        public ArrayList<Stem> allIntersection(int x1, int y1, int x2, int y2) {
